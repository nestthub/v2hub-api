...
Usage:
    ...
    # In endpoints
    from v2hub_api.middleware.rate_limit import check_public_rate_limit
...
from v2hub_api.core.exceptions import RateLimitError, to_http_exception
from v2hub_api.utils.rate_limiter import (
    RateLimitConfig,
    get_rate_limiter,
)
...
async def check_public_rate_limit(request: Request) -> None:
    ...
    if ban_service and await ban_service.is_banned(client_ip):
        ...
        if ban_info is None:
            raise RateLimitError(message="IP banned")
...
async def check_internal_rate_limit(request: Request) -> None:
    ...
    # 2. Check rate limit
    limiter = get_rate_limiter()
    is_allowed, error_message = await limiter.check_internal_limit(client_ip, api_token)
...
async def setup_rate_limiting(
    ...
    if not redis:
        logger.warning("Redis not available - rate limiting will fail open (allow all requests)")
