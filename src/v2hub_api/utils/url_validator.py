...
def validate_external_url(url: str) -> None:
    ...
    if parsed.scheme not in ("http", "https"):
        raise InvalidURLError(f"Invalid URL scheme: {parsed.scheme}. Only http/https allowed")
    ...
    if LOCALHOST_PATTERN.match(hostname):
        raise InvalidURLError("Localhost and 127.x.x.x addresses are not allowed")
    ...
    if re.match(r"^\d+\.\d+\.\d+\.\d+$", hostname) and is_private_ip(hostname):
        raise InvalidURLError(f"IP address {hostname} is private/local and not allowed")
