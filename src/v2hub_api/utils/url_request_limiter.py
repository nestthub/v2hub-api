...
class URLRequestLimiter:
    ...
    async def check_limit(self, ip: str) -> tuple[bool, str | None]:
        """
        ...
        try:
            ...
            if count >= self.requests_per_second:
                return (
                    False,
                    f"URL request rate limit exceeded: max {self.requests_per_second} per second",
                )
...
_url_request_limiter: URLRequestLimiter | None = None


async def get_url_request_limiter() -> URLRequestLimiter | None:
