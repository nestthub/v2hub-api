...
class WhitelistService:
    ...
    async def is_whitelisted(self, ip: str) -> bool:
        ...
        try:
            ...
            is_member = await cast(
                "Awaitable[int]",
                self.redis.sismember(self.whitelist_key, ip),
            )
            ...
            # Check CIDR ranges
            all_entries = await cast("Awaitable[Any]", self.redis.smembers(self.whitelist_key))

            for entry in all_entries:
                entry_str = entry.decode("utf-8") if isinstance(entry, bytes) else entry

                # If entry contains '/', it's a CIDR range
                if "/" in entry_str and self._ip_in_cidr(ip, entry_str):
                    return True
    ...
    async def add(self, ip_address: str, description: str | None = None) -> bool:
        """
        Add IP to whitelist.
        ...
        try:
            ...
            # Add to set
            added = await cast("Awaitable[Any]", self.redis.sadd(self.whitelist_key, ip_address))
            ...
            logger.info(
                f"IP added to whitelist: {ip_address} (%s)", description or "no description"
            )
    ...
    async def remove(self, ip_address: str) -> bool:
        ...
        try:
            # Remove from set
            removed = await cast("Awaitable[Any]", self.redis.srem(self.whitelist_key, ip_address))
    ...
    async def list_all(self) -> list[dict[str, Any]]:
        ...
        """
        try:
            entries = await cast("Awaitable[Any]", self.redis.smembers(self.whitelist_key))
            ...
            for entry in entries:
                ...
                # Get metadata
                meta_key = f"{self.whitelist_meta_prefix}{ip_address}"
                meta = await cast("Awaitable[Any]", self.redis.hgetall(meta_key))
    ...
    async def clear(self) -> int:
        ...
        try:
            ...
            # Delete the set
            await self.redis.delete(self.whitelist_key)
...
async def get_whitelist_service() -> WhitelistService | None:
