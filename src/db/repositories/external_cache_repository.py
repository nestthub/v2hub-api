...
from src.db.models import ExternalCache
...
class ExternalCacheRepository(BaseRepository[ExternalCache]):
    ...
    async def get_by_url_hash(self, url_hash: str) -> Optional[ExternalCache]:
        """Get cache entry by URL hash."""
        return await self.get_by_id(url_hash)
    
    async def get_by_url(self, url: str) -> Optional[ExternalCache]:
