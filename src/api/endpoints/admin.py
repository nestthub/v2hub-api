...
from src.services.whitelist_service import get_whitelist_service
from src.middlewares.rate_limit_middleware import get_client_ip
from src.schemas import (
    IPUnbanResponse, WhitelistAddResponse, WhitelistRemoveResponse,
    UserCreateResponse,UserResponse, UserStatusUpdateRequest,
    IPBanEntry, IPBanListResponse, IPBanRequest, IPBanStatusResponse,
    IPUnbanRequest, TokenRefreshRequest, TokenRefreshResponse,
    UserCreateRequest, WhitelistAddRequest, WhitelistEntry,
    WhitelistRemoveRequest, WhitelistListResponse,
)
...
async def verify_request_signature(
    ...
    if not x_signature or not x_timestamp:
        logger.warning(
            "Admin endpoint accessed without signature from IP %s",
    ...
    if not hmac.compare_digest(x_signature, expected_signature):
        logger.warning(
            "Invalid admin signature from IP %s for %s %s",
            get_client_ip(request),
            method,
            path
        )
    ...
    logger.info(
        "Admin request authenticated from IP %s for %s %s",
        get_client_ip(request),
        method,
        path
    )
...
async def verify_internal_ip(request: Request) -> None:
    ...
    if client_ip not in settings.admin_allowed_ips:
        logger.warning(
            "Admin endpoint accessed from unauthorized IP: %s",
            client_ip
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def create_user(
    ...
    try:
        ...
        logger.info(
            "User created: user_id=%d, user_hash=%s",
            user.user_id,
            user.user_hash
        )
...
async def ban_ip(
    ...
    if not ban_service:
        raise HTTPException(
            ...
            detail="Ban service not available"
        )
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to ban IP: {str(e)}"
        )
...
async def unban_ip(
    ...
    try:
        ...
        logger.info(
            "IP unban requested: %s (was_banned=%s)",
            request.ip_address,
            was_banned
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to unban IP: {str(e)}"
        )
...
async def list_banlist(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to list ban: {str(e)}"
        )
...
async def get_ban_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get ban status: {str(e)}"
        )
...
async def remove_from_whitelist(
    ...
    try:
        ...
        logger.info(
            "IP removed from whitelist: %s (existed=%s)",
            request.ip_address,
            removed
        )
