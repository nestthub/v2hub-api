...
class Settings(BaseSettings):
    ...
    api_token_length: int = Field(
        default=16,
        validation_alias="API_TOKEN_LENGTH"
    )
    ...
    public_rps: int = Field(
        default=3,
        description="Rate limit (requests per second) for public endpoints"
    )
    ...
    max_sources_per_subscription: int = Field(
        default=150,
        validation_alias="MAX_SOURCES_PER_SUBSCRIPTION"
    )
    ...
    @field_validator("cors_origins", "cors_methods", "cors_headers", "admin_allowed_ips", mode="before")
    @classmethod
